<script lang="ts">
	import { filteredScripts, scriptLoading, graphSelectedNode, flyoutOpen } from '$lib/stores';
...
<div class="p-6">
	...
	<div class="mb-8">
		...
		<p class="text-sm text-cs-text-muted">Manage, execute, and connect your scripts</p>
	</div>

	<!-- Quick Stats -->
	<div class="mb-8 grid grid-cols-4 gap-3">
		<div class="rounded-lg border border-cs-border bg-cs-surface p-4">
			<div class="mb-1 flex items-center gap-2 text-cs-text-muted">
		...
		<a href="/graph" class="rounded-lg border border-cs-border bg-cs-surface p-4 transition-colors hover:border-accent/30">
			<div class="mb-1 flex items-center gap-2 text-cs-text-muted">
				<GitBranch size={14} strokeWidth={2} />
				<span class="text-xs uppercase tracking-wider">Graph</span>
			</div>
			<span class="text-sm text-cs-text-muted">View Knowledge Graph</span>
		</a>
		<a href="/metrics" class="rounded-lg border border-cs-border bg-cs-surface p-4 transition-colors hover:border-accent/30">
			<div class="mb-1 flex items-center gap-2 text-cs-text-muted">
				<BarChart3 size={14} strokeWidth={2} />
				<span class="text-xs uppercase tracking-wider">Metrics</span>
			</div>
			<span class="text-sm text-cs-text-muted">Run Analytics</span>
		</a>
		<a href="/workflows" class="rounded-lg border border-cs-border bg-cs-surface p-4 transition-colors hover:border-accent/30">
			<div class="mb-1 flex items-center gap-2 text-cs-text-muted">
				<Zap size={14} strokeWidth={2} />
				<span class="text-xs uppercase tracking-wider">Workflows</span>
			</div>
			<span class="text-sm text-cs-text-muted">DAG Pipelines</span>
		</a>
	</div>

	<!-- Script Grid -->
	<h2 class="mb-3 text-xs font-medium uppercase tracking-wider text-cs-text-muted">All Scripts</h2>
	{#if $scriptLoading}
		<div class="grid grid-cols-3 gap-3">
			{#each Array(6) as _}
				<div class="animate-pulse rounded-lg border border-cs-border bg-cs-surface p-4">
					<div class="mb-2 h-4 w-2/3 rounded bg-cs-surface-2"></div>
					<div class="h-3 w-full rounded bg-cs-surface-2"></div>
				</div>
			{/each}
		</div>
	{:else}
		<div class="grid grid-cols-3 gap-3">
			{#each $filteredScripts as script}
				<button
					class="rounded-lg border border-cs-border bg-cs-surface p-4 text-left transition-all hover:border-accent/30 hover:bg-cs-surface-2"
				...
				>
					...
					<p class="mb-2 text-xs text-cs-text-muted line-clamp-2">
